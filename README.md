<!-- This documentation file explains usage, setup, or architecture details. -->
...
- Spring Data JPA repositories with custom JPQL queries (`search` for books and custom order fetch queries).
- Spring Security with:
  - In-memory `ADMIN` account (`admin@bookstore.local / Admin!234`).
  - Database-backed authentication via `AppUserDetailsService` (clients and employees).
  - URL and method-level authorization (`@PreAuthorize`).
  -    password encoding for create/update of users.
  - CSRF enabled (except H2 console endpoint).
- DTO validation, including custom `@StrongPassword` validation rule.
- Global exception handling with localized error messages and safe error output (no stack traces in responses).
- Localization resources for English and Ukrainian.
- Logging:
  - AOP-based service call and exception logging.
  - Service-level business logs in create/list operations.
- Search + pagination + sorting endpoint:
  - `GET /api/books/search?term=&page=0&size=10&sortBy=name`
- Frontend:
  - JavaScript Angular-based UI under `src/main/resources/static`.
  - Validation highlighting on form fields.
  - Language toggle in UI.
- Cloud readiness:
  - Production profile (`application-prod.properties`) with external PostgreSQL via env vars.
...
Or run jar with `SPRING_PROFILES_ACTIVE=prod` and `DB_URL`, `DB_USER`, `DB_PASSWORD`.
